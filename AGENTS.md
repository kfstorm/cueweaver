...
- The production container operates on user-mounted Media and Work directories; mounted Media must be readable and Work must be writable by the container user. E2E runs use `--user "$(id -u):$(id -g)"` so their bind-mounted test files remain owned by the test runner.
